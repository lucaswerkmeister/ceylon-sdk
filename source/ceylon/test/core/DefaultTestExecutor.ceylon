...
shared class DefaultTestExecutor(FunctionDeclaration functionDeclaration, ClassDeclaration? classDeclaration, Anything[] argumentList) satisfies TestExecutor {
        
    shared actual default TestDescription description => TestDescription(getName(), functionDeclaration, classDeclaration, argumentList);
    ...
    void invokeFunction(FunctionDeclaration f, Object? instance) {
        if (f.toplevel) {
            f.invoke([], *argumentList);
        } else {
            assert(exists i = instance);
            f.memberInvoke(i, [], *argumentList);
        }
    }
